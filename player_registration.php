...
if (isset($_POST['submit'])) {
    ...
    if ($result->num_rows == 0) {
        ...
        if (move_uploaded_file($file_source, $file_destination)) {
            $error = false;
            $insert_query = "INSERT INTO `players`(`first_name`, `last_name`, `gender`, `father_name`, `mother_name`, `address`, `district`, `phone_number`, `date_of_birth`, `photo_path`, `pin_code`, `email`, `password`) VALUES ('$first_name', '$last_name', '$gender', '$father_name', '$mother_name', '$address', '$district', '$phone_number', '$dob', '$file_destination', '$pin_code', '$email', '$password')";
            if (mysqli_query($connection, $insert_query)) {
                $error = false;
                header("location:player_registration_review.php?player_id=" . base64_encode(mysqli_insert_id($connection)));
            } else {
                $error = true;
                $error_message = 'Unable to register, please try again later';
            }
        } else {
            $error = true;
            $error_message = 'Unable to upload the file, please try again.';
        }
    } else {
        $error = true;
...
<main class="container">
    ...
    <?php } ?>
    <form id="player_registration_form" class="row border pt-2 p-3" action="player_registration.php" method="POST" enctype="multipart/form-data">
        <center>
            <h3 class="mb-3"> Player Registration Form </h3>
        </center>
        ...
        <div class="form-group text-center col-md-12 mb-3">
            <br>
            <input type="submit" name="submit" value="Register" class="btn w-50 btn-success">
        </div>
    </form>
</main>
